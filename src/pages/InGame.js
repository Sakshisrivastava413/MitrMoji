...
class InGame extends Component {
  constructor(props) {
	...
	componentDidMount() {
      ...
      .onSnapshot(res => {
        ...
        if (match.status == 'ENDED') {
          alert(`${match.winner.name} won the game!`);
	...
	showGlow(happy, afterScoreUpdated = () => {}) {
		this.setState({
			gameState: {
				...this.state.gameState,
	...
	feedEmojiRecogResult(emoji) {
		...
		if (emoji === this.state.gameState.expectedEmoji.text) {
          ...
          this.showGlow(
            ...
            () => {
              // end game condition
              if (this.state.gameState.score >= 200) {
                const playerKey = this.state.match.player1.name === this.state.user.displayName ? 'player1' : 'player2';
                firebase.firestore()
                  .collection('matchmaking')
                  .doc(this.state.match.timestamp)
                  .update({ status: 'ENDED', winner: this.state.match[playerKey] });
              }
            }
          );
